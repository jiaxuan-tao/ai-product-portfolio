# Reference｜Vibe Coding Prompt Library
...
本项目将 promptcat 的通用 Prompt 管理器方向改造成个人 Vibe Coding Prompt 模板库，重点服务 PRD、Codex 指令、UI 修改、Debug、README、Changelog、User Flow 和 Product Structure 等高频工作流。
...
- 保留 Prompt 管理器的基本信息结构：列表、分类、详情、复制和保存。

## 新增和调整的部分

- 将项目名称调整为 Vibe Coding Prompt Library。
- 将页面文案调整为 Vibe Coding 工作流语境。
- 将默认分类替换为 Vibe Coding 相关分类。
- 新增 8 个面向个人 Vibe Coding 的默认 Prompt 模板。
...
当前项目保留原始项目名称、GitHub 链接和 Demo 链接，并明确说明本项目是基于 promptcat 的轻量改造版本，而不是从零独立实现的项目。
