# Prompt Library 提示词资料库

## 项目简介

Prompt Library 提示词资料库是一个面向 Vibe Coding 工作流的本地化提示词管理工具，用于整理和复用 PRD、Codex 指令、UI 修改、问题修复和项目文档提示词。
...
![Prompt Library 提示词资料库中文界面预览](assets/vibe-coding-prompt-library-preview.png)
...
- 做：Prompt 分类、标签、收藏、模板浏览、搜索、复制和本地保存。
- 做：围绕 Vibe Coding 工作流重新组织默认内容。
- 不做：登录注册、云同步、团队协作、AI API、后端服务、支付订阅。
- 不做：复杂权限、复杂数据库、云端同步和多人数据管理。

## 使用方法

1. 打开 [app/index.html](app/index.html)，或按下方“本地运行”方式在浏览器中访问项目。
2. 在左侧选择提示词分类，或点击“收藏”查看已收藏的提示词。
3. 在中间列表选择模板，也可以通过搜索框查找所需内容。
4. 在右侧查看提示词正文，并按当前项目补充或修改标题、说明、标签和正文中的占位内容；勾选“收藏此提示词”可加入收藏。
5. 点击“复制内容”，将提示词粘贴到 Codex 或其他 AI 对话中使用。
6. 点击“保存提示词”保存自己的版本；也可以通过“创建副本”在默认模板基础上继续修改。
7. 点击“导出 JSON”下载当前资料库备份。导入备份时，点击“导入 JSON”选择文件；系统会合并合法记录，并跳过 ID 重复的内容。
8. 在左侧“自定义分类”中可以新增分类。重命名会同步更新分类内提示词；删除时必须选择迁移目标，提示词不会被删除。
9. 需要回到初始模板时，点击“恢复默认模板”。此操作会清除当前浏览器中保存的自定义内容、标签、收藏和自定义分类。
...
可以通过“导出 JSON”备份当前资料库，再在另一浏览器中使用“导入 JSON”恢复。导入只处理本地文件，不会上传任何数据；当前版本导出 `schemaVersion: 2` 格式，并兼容 v1 备份文件。导入采用合并导入、重复 ID 跳过的策略。
...
6. 增加 JSON 导入 / 导出，用于本地数据备份和迁移。
7. 增加标签、收藏和自定义分类，提升提示词沉淀与查找效率。
8. 补齐项目 README、PRD、用户流程、产品结构、Prompt 记录、changelog 和参考说明。
