...
8. 需要回到初始模板时，点击“恢复默认模板”。此操作会清除当前浏览器中保存的自定义内容。
...
可以通过“导出 JSON”备份当前资料库，再在另一浏览器中使用“导入 JSON”恢复。导入只处理本地文件，不会上传任何数据；当前版本使用 `schemaVersion: 1` 格式，并采用合并导入、重复 ID 跳过的策略。
...
7. 补齐项目 README、PRD、用户流程、产品结构、Prompt 记录、changelog 和参考说明。
