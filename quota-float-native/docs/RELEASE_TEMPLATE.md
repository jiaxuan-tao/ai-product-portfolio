# Quota Float Native 0.1.0

Quota Float Native is a lightweight floating desktop widget for checking Codex quota from the local Codex Desktop login state.

## Downloads

- Windows unsigned: `quota-float-native-windows-unsigned.zip`
- macOS Universal unsigned: `quota-float-native-macos-universal-unsigned.zip`
...
Quota Float Native does not store Codex tokens, account IDs, prompts, chats, raw quota responses, or local auth paths. It stores only widget preferences. See `PRIVACY.md`.
